...
def start_new_game(word_list, answer_word=None):
    ...
    new_game = {'answer_word':'',
                'masked_word':'',
                'previous_guesses':'',
                'remaining_misses': 5}
    if answer_word == None:
        new_game['answer_word'] = _get_random_word(word_list)
    else:
        new_game['answer_word'] = answer_word
    new_game['masked_word'] = _mask_word(new_game['answer_word'])    
    return new_game            
...
def guess_letter(game, letter):
    ...
    if letter in game['answer_word']:
        for num in range(len(game['answer_word'])):
            if letter == game['answer_word'][num]:
                masked_word_new += letter
            else:
                masked_word_new += game['masked_word'][num]
        game['masked_word'] = masked_word_new    
    else:
