...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    and guessed_letter not in previous_guesses\
    and guessed_letter.isalpha():
        return True
    else:
        return False
...
def start_new_game(word_list, answer_word=None):
    ...
    """
    if answer_word == None:
        game_answer = _get_random_word(word_list)
        game = {
            'answer_word': game_answer,
            'masked_word': _mask_word(game_answer),
            'previous_guesses': "",
            'remaining_misses': 5
        }
        return game
