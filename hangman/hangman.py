...
def start_new_game(word_list, answer_word=None):
    ...
    gameInfoDict = {}
    if answer_word:
        gameInfoDict['answer_word'] = answer_word
    else:    
        gameInfoDict['answer_word'] = _get_random_word(word_list)
    gameInfoDict['masked_word'] = _mask_word(gameInfoDict['answer_word'])
    gameInfoDict['previous_guesses'] = "" 
    gameInfoDict['remaining_misses'] = 5
...
def guess_letter(game, letter):
    ...
    if _guess_is_valid(letter, game['previous_guesses']):  #valid guess
        if letter in game['answer_word']:  #correct guess
            for i in range(len(game['answer_word'])):
                if game['answer_word'][i] == letter:
                    new_masked_string += letter
                else:
                    new_masked_string += game['masked_word'][i]
            game['masked_word'] = new_masked_string
        elif letter not in game['answer_word']:  #incorrect guess
