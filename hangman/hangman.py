...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    #previous_guesses = ''
    a_list = []
    if len(guessed_letter) == 1 and guessed_letter not in previous_guesses and str.lower(guessed_letter) in list(string.ascii_lowercase):
        return True
    return False
...
def start_new_game(word_list, answer_word=None):
    ...
    return {'answer_word' : answer_word, 'masked_word' : _mask_word(answer_word), 'previous_guesses' : '', 'remaining_misses' : 5}
...
def guess_letter(game, letter):
    ...
    if letter in game['answer_word']:
        for i in game['answer_word']:
            if i == letter:
                empty_answer_word += letter
            else:
                empty_answer_word += game['masked_word'][game['answer_word'].index(i)]
        game['masked_word'] = empty_answer_word
    elif letter not in game['answer_word']:
