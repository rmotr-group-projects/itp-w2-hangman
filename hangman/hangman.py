...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    """
    if len(guessed_letter) != 1 or not guessed_letter.isalpha() or guessed_letter in previous_guesses:
...
def _check_game_over(answer_word, masked_word, remaining_misses):
    ...
    :param remaining_misses: How many misses are left before user loses
    """
    if _check_win(answer_word, masked_word) == True:
        return True
...
def guess_letter(game, letter):
    ...
    if _guess_is_valid(letter, game['previous_guesses']):
        answer = list(game['answer_word'])
        mask = list(game['masked_word'])
        for characters in range(0, len(answer)):
            if letter == answer[characters]:
                mask[characters] = letter
                no_match = False
        if no_match:
                game['remaining_misses'] -= 1
        for i in range(0, len(mask)):
            new_mask_str += mask[i]
        game['masked_word'] = new_mask_str
