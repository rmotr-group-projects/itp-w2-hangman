...
def start_new_game(word_list, answer_word=None):
    ...
    if answer_word is not None:
        word_list = answer_word
    else:
        word_list = _get_random_word(word_list)
    
    game_dict = {
        'answer_word' : word_list,
        'masked_word' : _mask_word(word_list),
        'previous_guesses' : '',
        'remaining_misses' : 5,
    }
    return game_dict
...
def guess_letter(game, letter):
    ...
    if letter in game['answer_word']:
        ...
        for i in range(len(game['answer_word'])):
            correct_char = game['answer_word'][i]
            previously_guessed_char = game['masked_word'][i]
            if letter == correct_char:
                new_string += letter
            #elif previously_guessed_char != '*':
            #    new_string += previously_guessed_char
            else:
                new_string += previously_guessed_char
