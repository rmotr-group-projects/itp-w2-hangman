...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    if len(guessed_letter) == 1 and re.search('[a-zA-Z]', guessed_letter) and not(guessed_letter in previous_guesses):
        
        return True
    else:
        print ("Guess incorrect. Please choose 1 letter alphabet not previously guessed.")
        return False
...
def _check_lose(remaining_misses):
    ...
    if remaining_misses == 0:
        print("You have no remaining guess.")
        return True
    else:
        return False
...
def start_new_game(word_list, answer_word=None):
    ...
    print("Your word is:" + masked_word + ". 5 guesses remaining.")
    return {'answer_word' : answer_word,
...
def guess_letter(game, letter):
    ...
    if _guess_is_valid(letter,game['previous_guesses']):
        if letter in game['answer_word']:
            all_letters = game['previous_guesses'] + letter
            game['masked_word'] = re.sub('[^%s]' % all_letters,'*',game['answer_word'])
        else:
