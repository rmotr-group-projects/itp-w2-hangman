...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    """
    if len(guessed_letter) == 1 and guessed_letter.isalpha() == True and guessed_letter not in previous_guesses:
        return True
    else:
        return False
...
def _check_game_over(answer_word, masked_word, remaining_misses):
    ...
    :param remaining_misses: How many misses are left before user loses
    """
    obj1 = _check_win(answer_word, masked_word)
    obj2 = _check_lose(remaining_misses)
...
def guess_letter(game_values, letter):
    """
    ...
    this function.
    :param game_values: The dictionary storing current game information
    :param letter: The letter that is being guessed
    """

    new_string = ''
    if letter in game_values['answer_word']:
        for item in range(len(game_values['answer_word'])):
            if letter == game_values['answer_word'][item]:
                new_string += letter
            else: 
                new_string += game_values['masked_word'][item]
        game_values['masked_word'] = new_string   
    else:
        game_values['remaining_misses'] -= 1
    game_values['previous_guesses'] += letter
