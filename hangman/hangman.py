...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    """
    if (len(guessed_letter) == 1) and (guessed_letter not in previous_guesses) and (guessed_letter in string.ascii_letters):
        return True
    else:
        return False
...
def _check_game_over(answer_word, masked_word, remaining_misses):
    ...
    :param remaining_misses: How many misses are left before user loses
    """
    if _check_win(answer_word, masked_word) or _check_lose(remaining_misses) == True:
        return True
    else:
        return False
...
def start_new_game(word_list, answer_word=None):
    ...
    game = {"answer_word": answer_word, "masked_word": _mask_word(answer_word), 
...
def guess_letter(game, letter):
    ...
    if letter in game["answer_word"]:
        ...
        i = 0
        while i < len(game["answer_word"]):
            if game["answer_word"].count(letter, i) > 0:
                ind = game["answer_word"].index(letter, i)
                game["masked_word"] = game["masked_word"][:ind] + letter + game["masked_word"][(ind + 1):]
                i += 1
            else:
                break
    else:
