...
def _get_random_word(word_list):
    ...
    """
    answerword = random.choice(word_list) 
    return answerword
...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    """
    if guessed_letter.lower() in ('abcdefghijklmnopqrstuvwxyz'):
        if len(guessed_letter) == 1  and guessed_letter not in previous_guesses:
            return True 
        else:
            return False
    else:
...
def _check_game_over(answer_word, masked_word, remaining_misses):
    ...
    if _check_win(answer_word, masked_word) is True:
        return True
    elif _check_lose(remaining_misses) is True: #don't forget to save btw ;)
        return True
    else:
        return False
...
def guess_letter(game, letter):
    ...
    """
    replace_list = list(game['answer_word'])
    unecessary_list = []
    list3 = list(game['answer_word'])
    index = 0
    game['previous_guesses'] = game['previous_guesses'] + letter
    if letter not in game['answer_word']:
        game['remaining_misses'] -= 1
    else:
        for _ in game['answer_word']:
            if letter in list3:#not done! Just some beginning thinking! 
                unecessary_list = list(game['masked_word'])
                unecessary_list[list3.index(letter)+index] = letter
                game['masked_word'] = ''.join(unecessary_list)
                list3.remove(letter)
                index += 1
            else: 
                pass
