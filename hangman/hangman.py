...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    if len(guessed_letter) == 1 and not (guessed_letter in previous_guesses) and guessed_letter.isalpha():
        return not (guessed_letter in previous_guesses)
    else:
        return False
...
def start_new_game(word_list, answer_word=None):
    ...
    answer_word - The answer word masked with '*' characters
...
def guess_letter(game, letter):
    ...
    if letter in answer_word:
        masked_word = [char if char == letter or char in game['previous_guesses'] else "*" for char in answer_word]
