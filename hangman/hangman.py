...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    if len(guessed_letter) == 1 and guessed_letter.isalpha():
        if guessed_letter not in previous_guesses:
            return True
    return False
...
def guess_letter(game, letter):
    ...
    if letter in game['answer_word']:
        #newstr = game['masked_word']
        for a in range(len(game['masked_word'])):
            if game['answer_word'][a] == letter:
                newstr += letter
            else:
                newstr += game['masked_word'][a]
                    
        game['masked_word'] = newstr
...
# PYTHONPATH=. py.test -s tests -k test_guess_letter