...
def _guess_is_valid(guessed_letter, previous_guesses):
    ...
    if (len(guessed_letter) == 1 and 
        guessed_letter not in previous_guesses and 
...
def _check_game_over(answer_word, masked_word, remaining_misses):
    ...
    :param remaining_misses: How many misses are left before user loses
    """
    if (_check_win(answer_word, masked_word) == True or
        _check_lose(remaining_misses) ==  True):
        return True
    else:
        return False
...
def guess_letter(game, letter):
    ...
    if letter in game['answer_word']:
        for c in game['answer_word']:
            if c == letter:
                new_masked_word += letter
            else:
                new_masked_word += game['masked_word'][game['answer_word'].index(c)]
        game['masked_word'] = new_masked_word
        print(game['masked_word'])
    else:
        game['remaining_misses'] -= 1
        print("There are no {}\'s. {} misses remaining"
              .format(letter, game['remaining_misses']))
